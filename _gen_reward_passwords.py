...
- 仅含 2 字中文词，便于语音识别；输出 reward_passwords.js。
...
# 1) 读取现有 passwords.js，收集所有已用密码（用于去重）
used = set()
try:
    with open('passwords.js', 'r', encoding='utf-8') as f:
        text = f.read()
    # 匹配所有中文字符串值（语文/体育），以及英文/数字也一并收集以防万一
...
js = '// 自动生成的「隐藏奖励密码」表（2026-08-10 ~ 2027-01-31）\n'
js += '// 由 _gen_reward_passwords.py 生成，请勿手动修改。\n'
js += '// 每天 1 个，全局不重复；输入可得 2 积分。仅在家长设置中可见，作业打卡界面不显示。\n'
js += 'const DAILY_REWARD_PASSWORDS = {\n'
for k, v in out.items():
    js += f'"{k}": "{v}",\n'
js += '};\n'

with open('reward_passwords.js', 'w', encoding='utf-8') as f:
    f.write(js)

print(f'已生成 {days} 个隐藏奖励密码 -> reward_passwords.js')
