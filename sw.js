...
const CACHE = 'homework-garden-v48';
const ASSETS = [
  './',
  './index.html',
  './styles.css',
...
self.addEventListener('fetch', (e) => {
  ...
  // 每日变化的「数据」文件（密码表）：网络优先，保证当天内容最新；
  // 离线时回退到缓存，避免白屏。这样新增某天密码后，联网设备无需升 SW 版本也能即时取到，
  // 不再出现「GitHub 已更新、设备却一直用旧缓存」的错位。
  const isDailyData = url.pathname.endsWith('/passwords.js');
  if (isDailyData) {
    e.respondWith(
      fetch(e.request)
        .then((res) => {
