...
function defaultState() {
  ...
  return {
    ...
    manualPasswords: {}    // 家长手动设过密码的科目集合；其中的科目内置每日表不再覆盖
  };
}
...
function applyDailyPasswords() {
  ...
  if (!dp) return;
  const manual = state.manualPasswords || {};
  SUBJECTS.forEach((sub) => {
    if (manual[sub]) return;                 // 家长手动设过密码的科目，内置表不再覆盖
...
function openParentSettings() {
  ...
  const mp = state.manualPasswords || {};
  const SUBJECT_ICON = { '语文': '📖', '数学': '🔢', '英语': '🔤', '体育': '⚽' };
  const subjectFields = SUBJECTS.map((sub) => `
    <div class="field"><label>${SUBJECT_ICON[sub]} ${sub}密码 ${mp[sub] ? '<span class="manual-tag">🔒手动锁定</span>' : ''}</label><input data-pw="${sub}" value="${p[sub] || ''}" placeholder="例如：${sub}1" /></div>`).join('');
  ...
  box.innerHTML = `
    ...
    <div class="field" style="margin-top:6px">
      <div class="modal-actions">
        <button class="big-btn btn-yellow" data-action="restoreAuto">🔄 恢复每日自动密码（取消手动锁定）</button>
      </div>
      <div class="hint-sm">若某科后显示「🔒手动锁定」，说明此前在家长设置里手动设过密码，该科会锁定为旧密码、不再随日期自动更新。点上面按钮可一键恢复为每天自动更新的内置密码。</div>
    </div>
...
function saveParentSettings() {
  if (!state.manualPasswords) state.manualPasswords = {};
  document.querySelectorAll('#modalBox [data-pw]').forEach((inp) => {
    const sub = inp.dataset.pw;
    const val = inp.value;
    state.passwords[sub] = val;
    if (val && val.trim()) state.manualPasswords[sub] = true;   // 家长手动设过 → 标记，内置表不再覆盖
    else delete state.manualPasswords[sub];                     // 清空 → 交回内置每日密码
  });
...
function bindEvents() {
  ...
  document.getElementById('modalBox').addEventListener('click', (e) => {
    ...
    if (actBtn) {
      ...
      if (action === 'restoreAuto') {
        if (!state.manualPasswords) state.manualPasswords = {};
        const dp = (typeof DAILY_PASSWORDS !== 'undefined') ? DAILY_PASSWORDS[localDateStr()] : null;
        SUBJECTS.forEach((sub) => {
          delete state.manualPasswords[sub];                       // 取消手动锁定
          state.passwords[sub] = (dp && dp[sub] != null) ? String(dp[sub]) : '';  // 立刻回填当天内置密码
        });
        save();
        SUBJECTS.forEach((sub) => {
          const inp = document.querySelector('#modalBox [data-pw="' + sub + '"]');
          if (inp) inp.value = state.passwords[sub] || '';
        });
        document.querySelectorAll('#modalBox .manual-tag').forEach((el) => el.remove());
        showToast('已恢复为每日自动密码');
        return;
      }
    }
