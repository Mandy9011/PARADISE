...
function plantCareState(p, today) {
  ...
  const gap = dateDiff(today, p.lastWaterDate);
...
function petCareState(p, today) {
  ...
  const gap = dateDiff(today, p.lastFeedDate);
  if (gap <= 1) return { tier: 0, text: '🐾 生长状态良好', blank: false };
...
function ensureToday() {
  ...
  state.pets = (state.pets || []).filter((pt) => {
    ...
    if (dateDiff(today, pt.lastFeedDate) >= 4) {
      pt.starved = true;
...
function submitPassword(input) {
  ...
  const rw = (typeof DAILY_REWARD_PASSWORDS !== 'undefined') ? DAILY_REWARD_PASSWORDS[state.date] : null;
  if (rw && normalize(rw) === normalize(input)) {
    if (state.rewardClaimed.date === state.date && state.rewardClaimed.claimed) {
      showToast('🎁 今天的隐藏奖励已经领过啦');
      return;
    }
    state.lifePoints += 2;
    state.rewardClaimed = { date: state.date, claimed: true };
    addLog(state.date, fmtTime(new Date()), '隐藏奖励', '奖励 +2', 2);
    save();
    render();
    showToast('🎁 隐藏奖励！+2 积分');
    return;
...
function renderPets() {
  ...
  html += `<div class="tip-note">每天花 1 点喂食 1 次，每喂 3 次升 1 级，累计喂满 21 次就长成第 8 级永久宠物，之后就不用再喂食啦～连续 3 天不喂，宠物会进入「饿肚子」状态，再过 1 天就会消失哦！<br>${state.testMode ? '🧪 测试模式：可随时多次喂食、长按宠物名可删除。' : '✋ 长按宠物名可上下拖动排序。'}</div>`;
  ...
  html += `
    <div class="card pwd-area">
      ...
      <div class="tip-note">
        <b>📌 怎么打卡？</b>
        <ol class="hw-steps">
          <li>孩子完成一项作业，家长把对应的密码告诉他。</li>
          <li>在下方输入框：<b>按住 🎤 说话</b>（松开停止），或手动输入密码。</li>
          <li>点「✅ 验证」提交，验证通过就获得积分。</li>
        </ol>
        <b>⏰ 计分时段</b>（家长设置里可改）：${timeRulesText()}。<br>
        ${state.testMode ? '🧪 <b>测试模式开启</b>：不限制每科打卡次数与浇水次数，方便测试；正式使用请在家长设置关闭。' : '📌 每科每天只能打卡1次，每株花每天只能浇水1次，每只宠物每天只能喂食1次。表现优秀的时候会有隐藏奖励哦！'}
      </div>
    </div>`;
...
function renderGarden() {
  ...
  if (alive.length === 0) {
    ...
    alive.forEach((p) => {
      ...
      const picInner = care.blank ? '' : `${plantSvg(p)}<img class="plant-img" data-imgs="${cands}" data-i="0" src="${ibase}.jpg" alt="${p.type}" onerror="window.__tryImg(this)" />`;
      html += `
...
function openParentSettings() {
  ...
  const box = document.getElementById('modalBox');
  const rewardToday = (typeof DAILY_REWARD_PASSWORDS !== 'undefined') ? (DAILY_REWARD_PASSWORDS[state.date] || '') : '';
...
function openHelp() {
  showModal('❓ 怎么用？',
    `1️⃣ <b>作业打卡</b>：孩子完成作业后，点🎤说密码或手动输入，验证通过得积分。<br>
     2️⃣ <b>商店 / 我的花园</b>：用 3 点积分在商店播种喜欢的花，从第二天起每天花 1 点浇水，每浇 3 次升 1 级，浇满 21 次永久盛开留在花园！<br>
     &nbsp;&nbsp;⚠️ 连续 3 天不浇水，花朵会枯萎哦。<br>
     3️⃣ <b>宠物商店 / 我的宠物</b>：用 3 点领养宠物，每天花 1 点喂食 1 次，每喂 3 次升 1 级，喂满 21 次成为永久宠物，之后就不用再喂食啦。<br>
     &nbsp;&nbsp;⚠️ 连续 3 天不喂食，宠物会饿跑哦。<br>
...
function showPendingAlerts() {
  ...
  if (d.length && w.length) {
    ...
    showModal('📋 昨日小结', `昨天有 <b>${d.length}</b> 项作业没完成，扣了 <b>${d.length}</b> 点积分（${subs}）。<br>另外有 <b>${w.length}</b> 株植物枯萎了（${names}）。<br>今天加油哦！`, null);
  } else if (d.length) {
    const subs = d.map((x) => x.sub).join('、');
    showModal('📋 扣分提醒', `昨天有 <b>${d.length}</b> 项作业没完成，扣了 <b>${d.length}</b> 点积分：<br>${subs}。<br>今天早点完成就能拿高分啦！`, null);
  } else if (w.length) {
    const names = w.map((x) => x.type).join('、');
